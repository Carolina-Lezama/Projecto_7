...
if disp_button:
    ...
    fig = go.Figure(data=[go.Scatter(x=car_data['odometer'], y=car_data['price'], mode='markers+lines')])
...
chart = st.checkbox('Crear grafico de barras') #agregado por mi
...
if chart: #aqui usando px en lugar de go
    st.write('Creando grafico de barras')
    fig_bar = px.bar(car_data['type'].value_counts().reset_index(), x='index', y='type', title="Barras")
